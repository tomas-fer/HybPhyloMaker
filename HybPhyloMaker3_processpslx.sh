...
if [[ $cp =~ "yes" ]]; then
	if [ "$parallelmafft" = "yes" ]; then
		if [[ $location == "1" ]]; then
			...
			cat listOfFastaFiles.txt | parallel -j $numbcores 'mafft --auto {} > {}.mafft'
		fi
		# if [ ! $LOGNAME == "" ]; then
...
echo "Finished changing leading/tailing '-' in alignments..."
...
if [[ $cp =~ "no" ]]; then
	...
	echo "Finished concatenation of exons to loci..."
