...
#(7) heatmap of missing data per sample and locus for selected loci
...
if [[ $nrfew -gt 0 ]]; then
	...
	awk '{ if ($2 < 4) print $1 }' nrtaxa.txt | xargs -r rm 
	#delete all files with percentage of missing data for assemblies with less than 4 samples
	awk '{ if ($2 < 4) print $1 }' nrtaxa.txt | cut -d"_" -f 1,2 | sed "s/$/_${MISSINGPERCENT}percN.fas/" | xargs -r rm
fi
...
if [[ $location == "1" ]]; then
	#Run R script for boxplot/histogram visualization (run via xvfb-run to enable generating PNG files without X11 server)
	xvfb-run R --slave -f alignmentSummary.R >> R.log 2>&1
else
	R --slave -f alignmentSummary.R >> R.log 2>&1
fi
...
#Create heatmap of missing data for selected genes
echo -e "\nPlotting heatmap of missing data for selected loci ..."
mv MissingDataOverview_${MISSINGPERCENT}.txt MissingDataOverview.txt
#Modify loci names to names only
if [[ $corrected =~ "yes" ]]; then
	sed -i 's/CorrectedAssembly_//' MissingDataOverview.txt
else
	sed -i 's/Assembly_//' MissingDataOverview.txt
fi
if [[ $location == "1" ]]; then
	#Run R script for boxplot/histogram visualization (run via xvfb-run to enable generating PNG files without X11 server)
	xvfb-run R --slave -f heatmap.R >> R.log 2>&1
else
