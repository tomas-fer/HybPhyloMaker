...
# updated on      april		, 2014 by patrick kueck
...
sub argv_handling{
	...
	my $href_outfile_name	= $_[13];	# Outfilename of output option								-> IN (defined) / OUT (unchanged)
...
sub parameter{
	...
	# (2)
	unless (	$single_command	=~ /^s$|^i$|^f$|^q$|^h$|^n$|^p$|^a$|^c$|^o$|^a$|^b$|^q$|^d$|^e$|^g$|^j$|^k$|^l$/i ){ print "\n\tCOMMAND-ERROR!: Unknown command ".$single_command."!\n" }
...
sub start{
	...
	sub single_define{
		...
		my $aref_parameter_par	= $_[10];	# Print parsimonious sites as extra msa file				-> IN (defined) / OUT (unchanged)
		my $aref_parameter_ren	= $_[11];	# Rename taxon names of given ifiles						-> IN (defined) / OUT (unchanged)
		my $href_outfile_name	= $_[12];	# Outfilename of output option								-> IN (defined) / OUT (unchanged)
		my $aref_input_files		= $_[13];	# list of input files found in fas, FASTA, phy, or aln format						-> IN (defined) / OUT (changed)
		...
		for my $file ( sort @$aref_inputfile_list ){
			...
			for my $taxon ( sort keys %sequence_of_taxon ){
				...
				## (2) ...sequence type (nucleotide -> 'nu' is predefined. If amino acid unique sequence are found set sequence type of infile to amino acid -> 'aa'
					...
					if ( $sequence_of_taxon{$taxon}  =~ /I|E|L|Q|F|P/ ) { $href_hoh_info_of_infile_of_type->{$file}{seqtype} = 'aa' } # (2)
					##############################
				}
	...
	sub print_out{
		...
		my $sref_structure_seq					= $_[16];	# defined if structure sequence present in infiles										-> IN (defined) / OUT (unchanged)
		...
		sub info_all{
			...
			my $structure_seq							= $_[17];	# defined if structure sequence present in infiles										-> IN (defined) / OUT (unchanged)
			...
			## If supermatrix is printed out
			## (1) ...print infile name and supermatrix range of each infile and sequence infos
			## (2) ...print out output format of supermatrix and additional supermatrix info
			if ( $aref_parameter_fil->[0] =~ /Supermatrix/ ){
				...
				for my $state ( "N CODING CHARACTERS", "N INDELS", "N AMBIGUITIES", "N MISSING DATA [?]", "N GC CONTENT", "N PARSIMONY INFORMATIVE SITES"	){ print OUT_info "\t", $state }
				...
				for my $infile ( sort keys %$href_hol_seq_of_tax_of_infile ){
					...
					for my $state ( qw/infstate Ngapstate Nambstate Nmissingstate/		){
				...
				# (2)
				print OUT_info	"\n\nSUPERMATRIX FILE\tPRINTED AS FASTA\tPRINTED AS NEXUS\tPRINTED AS PHYLIP\tP CODING CHARACTERS [%]\tP INDELS [%]\tP AMBIGUITIES [%]\tP MISSING DATA [?] [%]\tP GC CONTENT [%]\tP PARSIMONY INFORMATIVE SITES [%]\nsupermatrix";
			...
			## (1) ...print infile name and sequence infos
				...
				# (1)
				print OUT_info "\n\nFile\tN SEQUENCES\tN SITES\tSEQUENCE TYPE\tN CHARACTERS" ;
				
				for my $state ( qw/A C G T U N Y R W S K M D V H B F L I P Q E/																				){ print OUT_info "\t", $state }
				for my $state ( "N CODING CHARACTERS", "N INDELS", "N AMBIGUITIES", "N MMISSING DATA [?]", "N GC CONTENT", "N PARSIMONY INFORMATIVE SITES"	){ print OUT_info "\t", $state }
		...
		sub raxml_out_prt{
			...
			if ( $$sref_partition_setting eq 'Supermatrix' ){
				...
				my $seqtype ;
				if ( $href_hoh_info_of_infile_of_type->{supermatrix}{seqtype} eq 'nu' ){ $seqtype = 'DNA' } else { $seqtype = 'WAG' }
