...
#$ -cwd
#$ -j y
#$ -N HybPipe1_rawprocess
#$ -o HybPipe1_rawprocess.log
...
cp $source/trimmomatic-0.32.jar .
#Make a new folder for results
mkdir $path/20filtered
#Create a reference PhiX index
echo -e "\nCreating PhiX reference...\n"
bowtie2-build PhiX.fsa phiX.index
...
do
	...
	samtools view -bT ../PhiX.fsa $file.sam > $file.bam
	rm $file.sam
	...
	if [[ $location == "1" ]]; then
		...
		java -d64 -server -XX:MaxHeapSize=10g -jar ../trimmomatic-0.32.jar PE -phred33 $file-noPhiX_1.fq.gz $file-noPhiX_2.fq.gz $file-1P $file-1U $file-2P $file-2U ILLUMINACLIP:../NEBNext-PE.fa:2:30:10 LEADING:20 TRAILING:20 SLIDINGWINDOW:5:20 MINLEN:36 >Trimmomatic.log 2>&1
	else
		java -jar ../trimmomatic-0.32.jar PE -phred33 $file-noPhiX_1.fq.gz $file-noPhiX_2.fq.gz $file-1P $file-1U $file-2P $file-2U ILLUMINACLIP:../NEBNext-PE.fa:2:30:10 LEADING:20 TRAILING:20 SLIDINGWINDOW:5:20 MINLEN:36 >Trimmomatic.log 2>&1
	fi	
...
tar cfz $path/20filtered/for_Geneious/$data-all-no-dups.tar.gz -C $path/20filtered/for_Geneious/ .
...
echo -e "Sample no.\tGenus\tSpecies\tNr. of pairs\tNr. of reads\tNr. of reads without PhiX\t% PhiX reads\tBoth suriving\tForward only surviving\tReverse only surviving\tNr. reads after quality trimming\t% quality trimmed reads\tNr. reads without duplicates\t% duplicates" > reads_summary.txt
