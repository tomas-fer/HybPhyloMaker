...
find -name "*.fasta" | sed 's/\.\///' > listOfFastaFiles.txt #to avoid 'Argument list too long' error
...
if [[ $cp =~ "yes" ]]; then
	...
	#rm *.fasta
	find -maxdepth 1 -name "*.fasta" -type f -delete #to avoid 'Argument list too long' error
...
if [[ $cp =~ "yes" ]]; then
	...
	find -name "*.fasta" | sed 's/\.\///' > listOfMAFFTFiles.txt #to avoid 'Argument list too long' error
else
