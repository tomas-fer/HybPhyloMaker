...
do
	...
	if [[ $location == "1" ]]; then
		...
		echo -e "\nProcessing file: ${file}"
		$fasttreebin -nt ${file}.fas > ${file}.fast.tre
	fi
...
if [[ $FastTreeBoot =~ "yes" ]]; then
	...
	do
		...
		if [[ $location == "1" ]]; then
			...
			raxmlHPC-SSE3 -f j -b 12345 -N 100 -s ${file}.fas -m GTRCAT -n BS
		else
			$raxmlseq -f j -b 12345 -N 100 -s ${file}.fas -m GTRCAT -n BS
		fi
		...
		if [[ $location == "1" ]]; then
			...
			$fasttreebin -nt ${file}.fas.BS${i} > ${file}.BS${i}.fast.tre
		fi
		done
