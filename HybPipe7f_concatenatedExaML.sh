#!/bin/bash
#PBS -l walltime=2d
#PBS -l nodes=4:ppn=4:infiniband
#PBS -j oe
#PBS -l mem=4gb
#PBS -l scratch=32gb:shared
#PBS -N HybPipe7f_ExaMLconcatenated_tree
#PBS -m abe
...
#Run ExaML
mpirun examl-AVX -t RAxML_parsimonyTree.RandomStartingTree -m GAMMA -s concatenated${MISSINGPERCENT}_${SPECIESPRESENCE}.partitioned.binary -n examl.tre
#Measure time
timenow=`date +%s`
...
raxmlHPC -N 100 -b 12345 -f j -m GTRCAT -s concatenated${MISSINGPERCENT}_${SPECIESPRESENCE}.phylip -q RAxMLpartitions.txt -n REPS
...
do
	...
	raxmlHPC -y -m GTRCAT -p 12345 -s concatenated${MISSINGPERCENT}_${SPECIESPRESENCE}.phylip.BS${i} -q RAxMLpartitions.txt.BS${i} -n T${i}
	#Prepare binary file for ExaML
	...
	#Run ExaML
	mpirun examl-AVX -s concatenated${MISSINGPERCENT}_${SPECIESPRESENCE}.phylip.BS${i}.binary -m GAMMA -t RAxML_parsimonyTree.T${i} -n BINF_${i}
	#Measure time
	timenow=`date +%s`
	timespan=`echo "scale=2;($timenow-$time)/60" | bc`
	if [[ $update =~ "yes" ]]; then
...
raxmlHPC -f b -m GTRGAMMA -z ExaML_bootstrap.tre -t ExaML_result.examl.tre -n ExaML_Bootstrap
